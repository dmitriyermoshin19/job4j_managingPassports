...
public class PassportServiceImpl implements PassportService {
    ...
    public PassportServiceImpl(PassportRepository repository, KafkaTemplate<Integer,
            Passport> kafkaTemplate) {
    ...
    @Override
//    @Scheduled(cron = "0 0 1 * * *") - every day in 1:00
    @Scheduled(fixedDelay = 10000)
    public String checkPassportByDate() {
        List<Passport> passports = findReplaceable();
        String result;
        if (!passports.isEmpty()) {
            result = " There are some passports for replacement: ";
            System.out.println(new Date() + result);
            for (Passport p : passports) {
                //в сервис рассылки почты
                kafkaTemplate.send("passport", p);
            }
        } else {
            result = "There are no passports for replacement";
